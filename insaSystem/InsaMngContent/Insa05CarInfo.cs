...
namespace insaSystem
{
    public partial class Insa05CarInfo : Form, IForm
    {
        ...
        private void TextboxClear()
        {
            car_com.Text = "";
            car_region.Text = "";
            car_yyyymm_f.Format = DateTimePickerFormat.Custom;
            car_yyyymm_t.Format = DateTimePickerFormat.Custom;
            car_pos.Text = "";
            car_dept.Text = "";
            car_job.Text = "";
            car_reason.Text = "";
        ...
        public void Btn_insert_clicked()
        {
            ...
            {
                TextboxClear();
                MessageBox.Show("경력사항 입력을 시작합니다.");
                InsaManagement.btncheck.Text = "C_I";
                InsaManagement.Mode = "BlockIUD";
            }
        }

        private void CallingEmployeeCarInfo()
        {
            if (bas_empno_car.Text == "")
            {
            ...
            InsaManagement.Mode = "BlockIUD";
            string car_sql = "";//sql문 작성해야함
            DataTable CarInfo = oHelper.GetData(car_sql);
            foreach (DataRow Row in CarInfo.Rows)
            {
                if (Row == null)
                {
